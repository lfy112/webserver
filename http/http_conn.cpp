...
const vector<string> NAMES = {"longfeiyu", "huangyan"};
...
http_conn::HTTP_CODE http_conn::do_request() {
  ...
  if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3')) {
    ...
    name[i - 5] = '\0';
    // printf("name:%s\n", name);
    // printf("%d", int(NAMES.size()));
    bool is_in_NAMES = false;
    for (int ii = 0; ii < NAMES.size(); ii++) {
      if (NAMES[ii].compare(name) == 0) {
        // printf("%s\n", name);
        is_in_NAMES = true;
        break;
      }
  ...
  if (*(p + 1) == 'u') {
    ...
    m_write_json_idx = strlen(jsonStr);
    strncpy(m_write_json_buf, jsonStr, strlen(jsonStr));
    return JSON_REQUEST;
  } else if (*(p + 1) == '0') {
    char *m_url_real = (char *)malloc(sizeof(char) * 200);
...
bool http_conn::add_status_line_json(int status, const char *title) {
  return add_response(
...
bool http_conn::process_write(HTTP_CODE ret) {
  switch (ret) {
    ...
    case JSON_REQUEST: {
      ...
      m_iv[1].iov_len = m_write_json_idx;
      m_iv_count = 2;
      bytes_to_send = m_write_idx + m_write_json_idx;
